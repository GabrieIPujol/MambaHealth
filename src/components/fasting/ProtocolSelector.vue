...
.protocol-item:hover:not(.disabled)  { border-color: var(--green-dim); }
.protocol-item.selected              { border-color: var(--green); background: var(--green-glow); }
.protocol-item.disabled              { opacity: .5; cursor: not-allowed; }

.protocol-name { font-family: var(--font-head); font-size: 1.1rem; font-weight: 600; letter-spacing: 1px; }
.protocol-desc { font-size: .75rem; color: var(--text-dim); margin-top: 2px; }

.protocol-right { display: flex; align-items: center; gap: 10px; }

.check { color: var(--text-dim); font-size: 1.1rem; }
.protocol-item.selected .check { color: var(--green); }
